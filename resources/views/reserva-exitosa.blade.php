...
<body class="bg-black text-white antialiased font-sans relative overflow-x-hidden min-h-screen flex items-center justify-center p-4">
  ...
  <div class="relative z-10 max-w-md w-full bg-zinc-900/80 border border-zinc-800 rounded-2xl p-8 backdrop-blur-md text-center shadow-2xl">
    ...
    <span class="text-xs font-bold text-amber-400 uppercase tracking-widest">¡Reserva Confirmada!</span>
    ...
    <div class="my-6 p-4 bg-white rounded-xl inline-block border-2 border-amber-500/40 shadow-lg">
      @if(!empty($qr_url))
        <img src="{{ $qr_url }}" alt="QR Reserva" class="w-44 h-44">
      @else
        <div class="w-44 h-44 flex items-center justify-center text-black text-xs text-center px-2">
          El código QR se está generando, revisa "Mis Reservas" en unos segundos.
        </div>
      @endif
    </div>
    ...
    <div class="bg-zinc-950/60 border border-zinc-800 rounded-xl p-4 text-left space-y-2 text-xs mb-4">
      <div class="flex justify-between border-b border-zinc-800/80 pb-2">
        ...
        <span class="font-mono font-bold text-amber-400">{{ $codigo_reserva ?? 'N/A' }}</span>
      </div>
      <div class="flex justify-between border-b border-zinc-800/80 pb-2">
        <span class="text-zinc-500">Mesa(s):</span>
        <span class="font-semibold text-white">{{ $mesa_id ?? 'N/A' }}</span>
      </div>
      <div class="flex justify-between border-b border-zinc-800/80 pb-2">
        <span class="text-zinc-500">Titular:</span>
        <span class="font-semibold text-white">{{ $nombre ?? 'N/A' }}</span>
      </div>
      <div class="flex justify-between border-b border-zinc-800/80 pb-2">
        <span class="text-zinc-500">Fecha:</span>
        <span class="font-semibold text-white">{{ $fecha ?? 'N/A' }}</span>
      </div>
      <div class="flex justify-between border-b border-zinc-800/80 pb-2">
        <span class="text-zinc-500">Zona:</span>
        <span class="font-semibold text-white uppercase">{{ str_replace('_', ' ', $zona ?? 'General') }}</span>
      </div>
      <div class="flex justify-between border-b border-zinc-800/80 pb-2">
        <span class="text-zinc-500">Total Pagado:</span>
        <span class="font-semibold text-amber-400">${{ number_format((float) ($precio ?? 0), 2) }} MXN</span>
      </div>
      <div class="flex justify-between">
        <span class="text-zinc-500">Método de Pago:</span>
        <span class="font-semibold text-white uppercase">{{ str_replace('_', ' ', $metodo_pago ?? 'Tarjeta') }}</span>
      </div>
    </div>
